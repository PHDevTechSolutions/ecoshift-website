...
export default function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    const activeCategoryNames = dbCategories.map((cat: any) =>
      cat.title?.trim().toUpperCase(),
    );
          ...
          product.technicalSpecs.forEach((spec: any) => {
            const foundRow = spec.rows?.find(
              (r: any) =>
                ...
                (key === "mountingType" &&
                  r.name.toLowerCase() === "mounting type"),
            );
            if (
              foundRow &&
              foundRow.value
      ...
      if (filters.fluxFrom || filters.fluxTo) {
        ...
        product.technicalSpecs?.forEach((spec: any) => {
          const row = spec.rows?.find(
            (r: any) =>
              r.name.toLowerCase().includes("lumen") ||
        ...
        <section className="py-24 bg-secondary/30">
          <div className="max-w-7xl mx-auto px-6">
            ...
            >
              ...
              <h1 className="font-serif text-5xl md:text-6xl mt-4 mb-6 leading-tight font-semibold">
                        ...
                        >
                          <div className="flex gap-6 items-center text-left">
                            ...
                            <div className="flex flex-col justify-center">
                              <h3
                                className={`font-serif text-lg md:text-xl transition-colors ${isOpen ? "text-emerald-700" : "text-foreground"}`}
                              >
                                {category.title}
                              </h3>
                                    ...
                                    >
                                      ...
                                      <div className="p-4 flex flex-col flex-1 border-t border-border/50">
                                        ...
                                        <div className="mt-auto">
                                          {/* CHANGED FROM <button> TO <div> TO FIX HYDRATION ERROR */}
                                          <div
                                            role="button"
                                            tabIndex={0}
                                            onClick={(e) => {
                                            ...
                                            className={`w-full py-2.5 rounded-lg text-xs font-semibold tracking-wide text-center transition-all cursor-pointer ${
                                              quoteCart.find(
                                                (item) =>
                                                  item.id === product.id,
                                              )
